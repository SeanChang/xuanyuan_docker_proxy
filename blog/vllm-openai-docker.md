...
![vllm-openai Docker 部署全手册](https://imgs.xuanyuan.cloud/docker/blog/docker-vllm.png)
...
| 参数                | 专业解释                                                                 |
|---------------------|--------------------------------------------------------------------------|
| `--tensor-parallel-size` | 张量并行数（1 表示用 1 张 GPU，若有 2 张 GPU 可改为 2，用于分摊大模型计算压力） |
| `--max-batch-size`   | 控制单次调度中允许的最大请求数，增大可提高吞吐，但会增加延迟与显存占用，需根据 GPU 显存调整 |
| `--temperature`      | 生成随机性（0~1，值越小输出越稳定，值越大输出越多样）|
| `--log-file`         | 日志输出路径，对应挂载的主机日志目录，便于持久化存储与排查问题             |
...
- “Out of memory”：减少 `--max-batch-size`，或启用量化参数降低显存占用。
...
- 解决：
  1.  启用 vLLM 推荐的权重量化（大幅降低显存占用），命令示例：
     ...
     docker run -d \
       ...
       --quantization awq
     ```
     说明：vLLM 推荐使用 AWQ / GPTQ 等权重量化格式，需模型本身支持对应量化格式（如模型文件名包含 awq/gptq 标识）。
...
- 性能优化：根据 GPU 数量调整 `--tensor-parallel-size`，根据业务需求启用 AWQ/GPTQ 量化或动态批处理。
