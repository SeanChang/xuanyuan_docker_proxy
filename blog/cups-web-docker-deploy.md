...
![Docker 部署 CUPS Web：浏览器即可管理打印机与打印任务](https://imgs.xuanyuan.cloud/docker/blog/cups-web.webp)
...
> 配图线上地址统一为 `![cups web {N}](https://imgs.xuanyuan.cloud/docker/blog/cups-web-{N}.webp)`（共 **21** 张）。

### 6.1 登录 Web 门户

打开 `http://192.168.1.10:1180`，使用默认账号 **`admin` / `admin`** 登录（页脚可见 **v0.2.3**）。**首次登录后请立刻修改默认密码。**

![CUPS Web 登录页：用户名 admin，页脚 v0.2.3](https://imgs.xuanyuan.cloud/docker/blog/cups-web-1.webp)

登录后进入「打印」主界面：可选打印机、标准/发票/身份证三种模式；左侧上传文件与打印参数，右侧预览与记录。此时若尚未添加打印机，「选择打印机」为空属正常。

![CUPS Web 主界面：标准打印，尚未选择打印机](https://imgs.xuanyuan.cloud/docker/blog/cups-web-3.webp)
...
![CUPS 首页：OpenPrinting CUPS 2.4.19](https://imgs.xuanyuan.cloud/docker/blog/cups-web-2.webp)

访问管理功能时浏览器会弹出登录框：用户名填 **`print`**（即 `.env` 的 `CUPSADMIN`），密码填你设置的 `CUPSPASSWORD`（本文为 `123456`）。

![CUPS 管理登录框：地址 192.168.1.10:631，用户名 print](https://imgs.xuanyuan.cloud/docker/blog/cups-web-4.webp)
...
![CUPS Administration：Add Printer 与服务器共享选项](https://imgs.xuanyuan.cloud/docker/blog/cups-web-5.webp)
...
![添加打印机：选中 Internet Printing Protocol (ipp)，发现列表为空](https://imgs.xuanyuan.cloud/docker/blog/cups-web-6.webp)
...
![添加打印机：Connection 填写 ipp://192.168.1.29/ipp/print](https://imgs.xuanyuan.cloud/docker/blog/cups-web-7.webp)

#### 6.2.4 命名并勾选共享

Name 例如 **`HP-Home`**，Description 随意；务必勾选 **Share This Printer**，否则 Web 门户列表看不到。点 **Continue**。

![添加打印机：名称 HP-Home，已勾选 Share This Printer](https://imgs.xuanyuan.cloud/docker/blog/cups-web-8.webp)

#### 6.2.5 选择厂商与型号

Make 选 **HP** → Continue。

![添加打印机：Make 选择 HP](https://imgs.xuanyuan.cloud/docker/blog/cups-web-9.webp)

Model 优先选列表顶部的 **IPP Everywhere™**（现代网络机通用），再点 **Add Printer**。若列表有具体型号亦可。

![添加打印机：Model 选择 IPP Everywhere](https://imgs.xuanyuan.cloud/docker/blog/cups-web-10.webp)

#### 6.2.6 默认选项与确认

进入 Set Default Options：Media Size 建议 **A4**，按需设置纸型与质量，点 **Set Default Options**。

![设置默认选项：Media Size 为 A4](https://imgs.xuanyuan.cloud/docker/blog/cups-web-11.webp)

成功提示：**Printer HP-Home default options have been set successfully.**

![CUPS 提示：HP-Home 默认选项设置成功](https://imgs.xuanyuan.cloud/docker/blog/cups-web-12.webp)
...
![打印机 HP-Home 详情：Idle Accepting Jobs Shared，IPP 地址 192.168.1.29](https://imgs.xuanyuan.cloud/docker/blog/cups-web-13.webp)
...
![CUPS Web：已选中 HP-Home，标准打印界面](https://imgs.xuanyuan.cloud/docker/blog/cups-web-14.webp)

除标准打印外，还可切换：

- **发票打印**：适合票据类多文件上传，可强制黑白等

![CUPS Web：发票打印模式](https://imgs.xuanyuan.cloud/docker/blog/cups-web-15.webp)

- **身份证打印**：分别上传正反面，可选 A4 / A5 排版

![CUPS Web：身份证打印，正反面上传区](https://imgs.xuanyuan.cloud/docker/blog/cups-web-16.webp)

**标准打印实测**：上传 `开户确认书.docx`（约 11 KB），系统提示 **已转换为 PDF，可以打印**；右侧出现 A4 预览。调好彩色/双面/份数后，点 **开始打印**。

![标准打印：Word 已转 PDF，右侧显示预览](https://imgs.xuanyuan.cloud/docker/blog/cups-web-17.webp)

提交成功会弹出 **打印任务已提交**（含任务 ID，如 `ipp://localhost:631/jobs/2`）。

![打印任务已提交：任务 ID 与页数提示](https://imgs.xuanyuan.cloud/docker/blog/cups-web-18.webp)

「打印记录」出现 **已打印** 条目；「打印机状态」可看到墨盒余量、纸盒信息等（取决于打印机上报能力）。

![打印记录显示已打印，墨盒信息 100%](https://imgs.xuanyuan.cloud/docker/blog/cups-web-19.webp)

![打印记录与打印机状态特写：空闲、队列 0、墨盒满](https://imgs.xuanyuan.cloud/docker/blog/cups-web-20.webp)

打印机吐出的纸件与预览一致，即整条链路跑通。

![实测出纸：开户确认书纸质件](https://imgs.xuanyuan.cloud/docker/blog/cups-web-21.webp)
