...
> 本文详细介绍MySQL容器化部署全流程，含4种镜像拉取方式、3种部署方案（快速部署适测试、挂载目录适生产、docker-compose适企业级）、3种结果验证手段及7类常见问题解决方案，还针对不同用户给出操作建议（如初学者先试快速部署，生产用挂载或compose）。
...
本文为 **MySQL Docker 单实例生产部署规范 v1.0**，详细介绍全流程操作，含4种镜像拉取方式、3 种部署方案（快速部署 *适合测试*、挂载目录 *适合单机生产*、docker-compose *适合企业入门级单实例*）、3种结果验证手段及7类常见问题解决方案，同时补充备份恢复、安全基线、HA 选型建议，适配 SaaS 初创、内部系统、中小公司及私有部署客户场景。
...
企业生产环境建议先审计脚本内容，再在受控环境执行，避免供应链风险。可通过 `wget https://xuanyuan.cloud/docker.sh && less docker.sh`（备用地址1：`wget https://get.xuanyuan.dev/docker.sh && less docker.sh`，备用地址2：`wget https://get.xuanyuan.me/docker.sh && less docker.sh`） 查看脚本逻辑，确认无异常后再运行。
...
⚠️ **镜像名必须与 `docker run` 保持一致**（常见踩坑）：
- 若只执行了 `docker pull docker.xuanyuan.run/library/mysql:8.0`，却直接 `docker run ... library/mysql:8.0`，Docker 会尝试从 Docker Hub 拉取 `mysql:8.0`，国内环境可能长时间卡住。
- **推荐做法（二选一）**：
  1. **全程使用轩辕完整镜像名**（最简单，无需 tag）：`docker run ... docker.xuanyuan.run/library/mysql:8.0`
  2. **先 tag 再 run 短名**：按 2.2 节完成 `docker tag` 后，再使用 `library/mysql:8.0` 或 `mysql:8.0`
- 若已配置 `/etc/docker/daemon.json` 的 `registry-mirrors` 指向轩辕镜像，可直接使用 `mysql:8.0` 短名。
...
⚠️ 以下示例使用轩辕完整镜像名，与 2.1 节 `docker pull` 保持一致；若已按 2.2 节完成 tag，可替换为 `library/mysql:8.0`。
...
**验证方式**：
MySQL 首次启动需初始化数秒，**先确认容器处于运行状态**，再进入终端：
```bash
# 1. 确认 STATUS 为 Up（若为 Exited，先看日志排查）
docker ps | grep mysql-test

# 2. 若容器未运行，查看退出原因
docker logs mysql-test
docker inspect mysql-test --format='{{.State.ExitCode}} {{.State.Error}}'

# 3. 容器正常运行后，进入 MySQL 终端
docker exec -it mysql-test mysql -uroot -p
```
输入密码 `123456` 后，若能进入 MySQL 命令行，则部署成功。

若提示 `container ... is not running`，说明容器已退出，**不要重复 `docker run` 同名容器**（会报 name already in use），应先 `docker rm -f mysql-test` 再排查日志后重试。
...
⚠️ **首次部署时 `/data/mysql/data` 必须为空目录**。MySQL 官方镜像仅在 `/var/lib/mysql` 为空时执行初始化；若目录中已有上次失败留下的残留文件，容器会反复退出（详见 5.3 节）。确认空目录：
```bash
...
**方式一**（绑定宿主机内网 IP，替换 `192.168.1.10` 为真实内网 IP）：
```bash
...
**方式二**（不做端口映射，仅 Docker 内部网络访问，删除 `-p` 行即可）：
```bash
docker run -d --name mysql-prod \
  -e MYSQL_ROOT_PASSWORD=StrongPass123! \
  -e TZ=Asia/Shanghai \
...
#### 第一步：创建目录并放置 compose 文件
在宿主机创建目录，并将 `docker-compose.yml` 放在该目录下（下文 volumes 使用相对路径，**必须与 yml 同目录**）：

```bash
mkdir -p /data/mysql/{conf,data,logs,backup}
cd /data/mysql
# 将 docker-compose.yml 创建在本目录（与 conf、data、logs、backup 同级）
```

⚠️ **路径说明**：3.2 节 `docker run` 使用绝对路径 `/data/mysql/...`；3.3 节 compose 使用相对路径 `./data` 等。若 compose 文件放在 `/data/mysql/`，两者实际指向同一目录。切勿在 A 目录执行 `mkdir /data/mysql/...` 却在 B 目录放 yml 文件。

⚠️ **权限说明**：MySQL 官方镜像容器内运行用户 UID 通常为 **999**（`mysql` 用户），entrypoint 会自动处理数据目录权限，**一般无需**手动 `chown 1000:1000`。空目录直接启动即可；SELinux 场景参考 3.2 节 `chcon` 命令。

#### 第二步：编写 docker-compose.yml 文件
在 `/data/mysql/` 目录下创建 `docker-compose.yml`，内容如下（含资源限制、安全配置、时区设置）：
```yaml
version: '3.8'  # 兼容的 docker-compose 版本
services:
  mysql:
    # ⚠️ 8.0 为浮动tag，小版本升级可能引入行为变化，生产建议固定到具体小版本
    image: docker.xuanyuan.run/library/mysql:8.0.36  # 生产推荐：固定小版本；ARM 改为 arm64v8/mysql:8.0.36；老 CPU 见 5.2 节
    ...
    volumes:
      # 目录挂载（宿主机目录:容器目录，相对路径与 yml 同目录）
      - ./conf:/etc/mysql/conf.d       # 配置文件目录
      - ./data:/var/lib/mysql          # 数据文件目录（首次启动必须为空）
      - ./logs:/var/log/mysql          # 日志文件目录
      - ./backup:/backup               # 备份输出目录（勿挂到 /var/lib/mysql 子目录，见下方说明）
      - ./my.cnf:/root/.my.cnf         # 客户端配置文件，供 mysqldump 使用
    deploy:
      resources:
        limits:
          cpus: '2'  # 限制最大使用2核CPU
...
⚠️ **备份目录挂载避坑**：**禁止**将 backup 挂载到 `/var/lib/mysql/backup`（数据目录的子路径）。Docker 会在 `./data` 下自动创建 `backup` 挂载点，导致数据目录非空，MySQL 首次初始化失败（报 `data directory has files in it`）。备份目录应挂载到独立路径（如 `/backup`），与 `./data` 同级。

#### 第三步：启动服务
在 `docker-compose.yml` 所在目录（`/data/mysql/`）执行以下命令：

⚠️ 首次启动前确认 `./data` 目录为空（仅含 `.` 和 `..`），**不含 `backup` 子目录**，否则可能触发 5.3 节的数据目录初始化失败。若此前误用过 `/var/lib/mysql/backup` 挂载，需先清理：
```bash
docker compose down
rm -rf ./data/*
ls -la ./data   # 应仅有 . 和 ..
...
### 4.2 容器状态验证
查看容器是否处于运行状态（**执行 `docker exec` 前必须先确认容器在运行**）：
```bash
docker ps
docker ps -a   # 若未在运行，查看 Exited 容器的退出状态
```
若 `STATUS` 列显示 `Up`（如 `Up 5 minutes`），则容器运行正常；若为 `Exited`，请查看 4.3 节日志并按第 5 章排查。
...
- **容器名冲突**：重复执行 `docker run --name mysql-test` 会报 `name already in use`，先执行 `docker rm -f mysql-test` 再重试。

### 5.2 容器启动后立即退出（ExitCode 127）？
若 `docker logs` 出现以下错误，说明 **CPU 不支持 x86-64-v2**，与密码、端口无关：
```text
Fatal glibc error: CPU does not support x86-64-v2
```
**原因**：Docker Hub 较新的 MySQL 8.0 官方镜像已要求 x86-64-v2 指令集（SSE4.2、POPCNT 等），老旧物理 CPU 或虚拟机 CPU 类型设为过保守（如 `qemu64`）时会触发。

**排查**：
```bash
grep -m1 flags /proc/cpuinfo   # 若无 sse4_2、popcnt 等标志，即不支持
docker inspect mysql-test --format='{{.State.ExitCode}}'
docker logs mysql-test
```

**解决方案（按优先级）**：
1. **换兼容镜像**：试 `docker.xuanyuan.run/library/mysql:5.7`，或固定较旧的 8.0 小版本（如 `8.0.32`）
2. **改用 MariaDB**：`docker.xuanyuan.run/library/mariadb:10.11`（连接命令为 `mariadb` 而非 `mysql`）
3. **调整虚拟机 CPU 类型**（若在 VM 中）：Proxmox/KVM 改为 `host` 或 `x86-64-v2-AES`，重启 VM 后再试

### 5.3 数据目录非空导致初始化失败？
若 `docker logs` 出现以下错误，说明挂载的数据目录 **不为空或含无效残留文件**：
```text
[ERROR] [MY-010457] [Server] --initialize specified but the data directory has files in it. Aborting.
[ERROR] [MY-013236] [Server] The designated data directory /var/lib/mysql/ is unusable.
```
**原因**：MySQL 官方镜像 **仅在 `/var/lib/mysql` 为空目录时** 才执行初始化。常见触发场景：
- 首次部署前 `/data/mysql/data` 或 compose 的 `./data` 已有文件（如手动创建的其他文件）
- **docker-compose 将 backup 误挂到 `/var/lib/mysql/backup`**，Docker 会在 `./data/backup` 创建挂载点，导致数据目录非空（见 3.3 节）
- 上次容器启动失败，在数据目录留下了未完成的初始化残留
- 切换 MySQL 版本后复用旧数据目录，文件格式不兼容

**排查**：
```bash
docker ps -a | grep mysql
docker logs mysql-prod          # 或 mysql-test / mysql-service
ls -la /data/mysql/data         # docker run 方式
ls -la ./data                   # docker compose 方式（在 yml 所在目录执行）
# 若 data 下出现 backup 子目录，说明 backup 挂载路径配置有误，见 3.3 节
```

**解决方案**：
1. **全新安装（无重要数据）**：停止并删除容器后，清空数据目录再重启：
   ```bash
   docker rm -f mysql-prod
   rm -rf /data/mysql/data/*
   # 确认目录为空后再 docker run / docker compose up -d
   ```
2. **已有有效数据**：不要清空目录，应使用与数据匹配的 MySQL 版本镜像直接启动（跳过 `--initialize`）
3. **docker-compose 场景**：修正 yml 中 backup 挂载为 `./backup:/backup`（勿挂到 `/var/lib/mysql/backup`），然后清空 `./data` 后重新 `docker compose up -d`（⚠️ 会丢失 data 目录内所有数据，操作前确认无重要数据）

### 5.4 如何初始化数据库？
⚠️ MySQL 官方镜像仅在 `/var/lib/mysql` 为空目录时执行初始化逻辑。若目录中已存在有效数据，`MYSQL_*` 环境变量（如创建数据库、用户）将被忽略，需手动通过 SQL 命令操作（非空目录报错见 5.3 节）。
...
3. 创建备份脚本 `/data/mysql/backup/backup.sh`（compose 部署时 `BACKUP_DIR` 改为 `/data/mysql/backup` 或脚本所在目录）：
```bash
#!/bin/bash
# MySQL 定时备份脚本（无明文密码版）
# mysql-service 为示例容器名，请替换为实际容器名称
BACKUP_DIR=/data/mysql/backup   # compose 方式：与 yml 中 ./backup 挂载的宿主机目录一致
...
4. **事故4：挂载目录非空导致 MySQL 反复初始化失败**
   根因：① 首次部署时数据目录已有残留文件；② **compose 将 backup 挂到 `/var/lib/mysql/backup`**，Docker 在 `./data/backup` 创建挂载点，MySQL 判定数据目录非空后报 `data directory has files in it` 并退出。
   解决方案：backup 改为独立挂载（如 `./backup:/backup`）；全新部署前确认 `./data` 为空；失败后 `docker compose down` 并 `rm -rf ./data/*` 再重试（无重要数据时）；有数据时勿清空，改用匹配版本的镜像启动。
