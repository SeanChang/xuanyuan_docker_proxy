...
> 在线版：https://xuanyuan.cloud/usage/login
...
- [搜索镜像](#搜索镜像)
- [流量耗尽错误提示](#流量耗尽错误提示)
- [UNAUTHORIZED 登录失败](#unauthorized-登录失败)
- [410 错误问题](#410-错误问题)
- [manifest unknown 错误](#manifest-unknown-错误)
...
> **注意**：**镜像账户**和**镜像密码**可在[登录](https://xuanyuan.cloud/dashboard?next=%2Fusage%2Flogin)后，在左侧菜单栏的「个人中心」中的**「用户信息」**页面中的**「镜像仓库信息」**卡片中查看。登录后系统会自动为您创建这些凭据。
...
示例（请将 镜像账户、镜像密码 替换为个人中心中的实际值；在 Windows CMD 中请勿给密码加双引号）：
...
```bash
docker pull ***.xuanyuan.run/镜像路径:标签
```

（请将 `***.xuanyuan.run` 替换为您的专属域名，可在控制台左侧菜单栏找到）
