...
> 在线版：https://xuanyuan.cloud/usage/nologin
...
- [专属域名方式介绍](#专属域名方式介绍)
- [获取专属域名](#获取专属域名)
- [Docker Hub（docker.io）](#docker-hubdockerio)
- [GitHub Container Registry（ghcr.io）](#github-container-registryghcrio)
- [Google Container Registry（gcr.io）](#google-container-registrygcrio)
- [Quay.io](#quayio)
- [NVIDIA Container Registry（nvcr.io）](#nvidia-container-registrynvcrio)
- [Kubernetes Registry（registry.k8s.io）](#kubernetes-registryregistryk8sio)
- [Microsoft Container Registry（mcr.microsoft.io）](#microsoft-container-registrymcrmicrosoftio)
- [Elastic Registry（docker.elastic.co）](#elastic-registrydockerelasticco)
- [Oracle Container Registry（container-registry.oracle.com）](#oracle-container-registrycontainer-registryoraclecom)
- [GitLab Container Registry（registry.gitlab.com）](#gitlab-container-registryregistrygitlabcom)
- [使用专属域名搜索镜像](#使用专属域名搜索镜像)
- [常见用法建议](#常见用法建议)
- [避免的问题](#避免的问题)
- [流量耗尽错误提示](#流量耗尽错误提示)
...
专属域名方式让您无需执行 docker login 命令，直接通过专属地址拉取镜像，更加便捷高效。您需要先 [登录](https://xuanyuan.cloud/dashboard?next=%2Fusage%2Fnologin) 网站，然后点击左侧菜单栏的「专属域名」菜单生成专属的专属域名镜像仓库地址。

> 本教程默认您的专属域名为 `***.xuanyuan.run`（请将 `***.xuanyuan.run` 替换为您的专属域名。[登录](https://xuanyuan.cloud/dashboard?next=%2Fusage%2Fnologin) 网站后，点击左侧菜单栏的「专属域名」菜单即可获取）

## 获取专属域名

[登录](https://xuanyuan.cloud/dashboard?next=%2Fusage%2Fnologin) 后，在左侧菜单栏的「专属域名」菜单中点击"显示专属域名镜像仓库地址"按钮，系统将为您生成专属的专属域名。
...
> **拉取成功后**，如果你想去掉前缀 `***.xuanyuan.run`，请使用以下命令：

```bash
# 1. 拉取镜像（带 轩辕镜像 域名）
docker pull ***.xuanyuan.run/library/nginx:1.27.3-alpine

# 2. 打上一个本地新的 tag（去掉域名前缀）
docker tag ***.xuanyuan.run/library/nginx:1.27.3-alpine nginx:1.27.3-alpine

# 3. 删除原来的带域名的 tag（可选）
docker rmi ***.xuanyuan.run/library/nginx:1.27.3-alpine
```

这样就可以去掉 `***.xuanyuan.run` 域名前缀了。
...
```bash
docker pull ***-nvcr.xuanyuan.run/nvidia/pytorch:23.05-py3
```

> ⚠️ 私有镜像仍需登录，详见官网获取 API Key 或使用企业授权
...
```bash
docker pull ***-k8s.xuanyuan.run/kube-apiserver:v1.30.1
```
...
```bash
docker pull ***-mcr.xuanyuan.run/dotnet/runtime:8.0
```
...
```bash
docker pull ***-elastic.xuanyuan.run/elasticsearch/elasticsearch:8.13.4
```
...
```bash
docker pull ***-oracle.xuanyuan.run/database/enterprise:21.3.0
```

> ⚠️ Oracle 镜像需先登录授权，详见 Oracle 官网说明。
...
```bash
docker pull ***-gitlab.xuanyuan.run/gitlab-org/gitlab-runner:latest
```

> ⚠️ 仅支持 registry.gitlab.com（SaaS）公共镜像；GitLab 私有项目镜像不支持通过专属域名拉取。
...
```bash
docker search ***.xuanyuan.run/mysql
```
