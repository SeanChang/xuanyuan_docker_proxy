...
> 在线版：https://xuanyuan.cloud/usage/apple-container
...
> **注意**：若你仍在使用 Docker 引擎，请参考 [OrbStack 教程](https://xuanyuan.cloud/usage/orbstack) 或 [Docker Desktop 教程](https://xuanyuan.cloud/usage/desktop)。
...
> **注意**：请将 ***.xuanyuan.run** 替换为您的专属域名。[登录](https://xuanyuan.cloud/dashboard?next=%2Fusage%2Fapple-container)网站后，点击左侧菜单栏的「专属域名」即可获取。

Docker Hub 镜像的完整拉取路径格式为：`***.xuanyuan.run/library/镜像名:标签`。更多仓库前缀规则请参考 [专属域名拉取教程](https://xuanyuan.cloud/usage/nologin)。

## 拉取镜像

> ⚠️ 必须指定 `--platform linux/arm64`，否则可能拉取全部架构，浪费流量与磁盘空间。
...
```dockerfile
FROM ***.xuanyuan.run/library/node:20
...
> **注意**：正确做法：在 pull / run / build 命令中始终使用轩辕专属域名**完整镜像路径**，例如 `***.xuanyuan.run/library/nginx:latest`。
...
| GHCR / GCR 等其他仓库怎么拉 | 不同仓库前缀规则不同 | 参考 [专属域名拉取教程](https://xuanyuan.cloud/usage/nologin) 中的多仓库前缀对照 |

## 相关教程

- [专属域名拉取教程](https://xuanyuan.cloud/usage/nologin) — GHCR、GCR、Quay 等多仓库前缀对照与拉取示例
- [多仓库镜像教程总览](./mirror-tutorial-docker-guide.md) — 各主流仓库独立配置文档入口
- [OrbStack 教程](https://xuanyuan.cloud/usage/orbstack) — 若你仍在使用 Docker 引擎，可配置全局 `registry-mirrors`
