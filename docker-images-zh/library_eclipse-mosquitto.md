...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/library/eclipse-mosquitto" title="library/eclipse-mosquitto Docker 镜像中文简介、标签列表与拉取命令">library/eclipse-mosquitto 中文简介</a>
...
2. 运行容器时，将本地配置目录挂载到容器的 `/mosquitto/config`：  
   ```bash
   docker run -it -p 1883:1883 -v "$PWD/mosquitto/config:/mosquitto/config" docker.xuanyuan.run/eclipse-mosquitto
   ```  
...
docker run -it -p 1883:1883 \
  ...
  docker.xuanyuan.run/eclipse-mosquitto
```  
