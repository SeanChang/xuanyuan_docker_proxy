...
```bash
docker run -d -p 8081:8081 --name nexus docker.xuanyuan.run/sonatype/nexus3
```
...
docker run -d -p 8081:8081 --name nexus -v nexus-data:/nexus-data docker.xuanyuan.run/sonatype/nexus3 # 启动时挂载
```
...
docker run -d -p 8081:8081 --name nexus -v /some/dir/nexus-data:/nexus-data docker.xuanyuan.run/sonatype/nexus3 # 挂载启动
```
...
5. **环境变量配置**：  
   - `INSTALL4J_ADD_VM_PARAMS`：设置 JVM 参数（默认 `-Xms2703m -Xmx2703m -XX:MaxDirectMemorySize=2703m -Djava.util.prefs.userRoot=${NEXUS_DATA}/javaprefs`），例如：  
     ```bash
     docker run -d -p 8081:8081 --name nexus -e INSTALL4J_ADD_VM_PARAMS="-Xms1g -Xmx1g" docker.xuanyuan.run/sonatype/nexus3
     ```  
   - `NEXUS_CONTEXT`：设置访问上下文路径（默认 `/`），例如：  
     ```bash
     docker run -d -p 8081:8081 --name nexus -e NEXUS_CONTEXT=nexus docker.xuanyuan.run/sonatype/nexus3 # 访问路径为 [] ```  
