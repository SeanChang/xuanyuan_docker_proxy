...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/jenkins/jenkins" title="jenkins/jenkins Docker 镜像中文简介、标签列表与拉取命令">jenkins/jenkins 中文简介</a>
...
- **最新 LTS 版**（长期支持版）：  
  `docker pull docker.xuanyuan.run/jenkins/jenkins:lts-jdk17`  

- **最新每周版**（包含最新特性）：  
  `docker pull docker.xuanyuan.run/jenkins/jenkins:jdk17`  

- **其他轻量级版本**：  
  同时提供基于 alpine 系统、Windows 系统及其他 JDK 版本的轻量级镜像。  

- **推荐：固定特定版本**（避免自动更新导致问题）：  
  格式为 `docker pull docker.xuanyuan.run/jenkins/jenkins:<version>-<jdk>`，例如：  
