...
### 使用CPU版本的Docker镜像
```bash
docker run --name dev -v $PWD:/mnt -p 8888:8888 -it docker.xuanyuan.run/paddlecloud/paddledetection:2.4-cpu-e9a542 /bin/bash
```

### 使用GPU版本的Docker镜像
```bash
docker run --name dev --runtime=nvidia -v $PWD:/mnt -p 8888:8888 -it docker.xuanyuan.run/paddlecloud/paddledetection:2.4-gpu-cuda10.2-cudnn7-e9a542 /bin/bash
```
