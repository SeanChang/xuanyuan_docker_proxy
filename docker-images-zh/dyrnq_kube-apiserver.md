...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/dyrnq/kube-apiserver" title="dyrnq/kube-apiserver Docker 镜像中文简介、标签列表与拉取命令">dyrnq/kube-apiserver 中文简介</a>
...
命令格式通常是：
```bash
docker pull ***-k8s.xuanyuan.run/kube-apiserver:<版本标签>
```
...
例如，拉取 v1.28.0 版本：
```bash
docker pull ***-k8s.xuanyuan.run/kube-apiserver:v1.28.0
```
...
1.  **手动测试或调试（不推荐生产）**：
    ...
    ```bash
    docker run --rm ***-k8s.xuanyuan.run/kube-apiserver:v1.28.0 --etcd-servers=[] --tls-cert-file=/path/to/cert --tls-private-key-file=/path/to/key ...
    ```
