...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/papermerge/papermerge" title="papermerge/papermerge Docker 镜像中文简介、标签列表与拉取命令">papermerge/papermerge 中文简介</a>
...
docker run -p 8000:8000 \
    ...
    docker.xuanyuan.run/papermerge/papermerge:latest
```
...
services:
  app:
    image: docker.xuanyuan.run/papermerge/papermerge
    environment:
    ...
    depends_on:
      - db
  db:
    image: docker.xuanyuan.run/bitnami/postgresql:14.4.0
    volumes:
