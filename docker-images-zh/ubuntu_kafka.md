...
### 2. 部署 ZooKeeper  
```bash  
docker run --detach --name zookeeper --network kafka-network docker.xuanyuan.run/ubuntu/zookeeper:3.8-22.04_edge
```  

### 3. 部署 Kafka  
```bash  
docker run --detach --name kafka --network kafka-network -p 9092:9092 docker.xuanyuan.run/ubuntu/kafka:3.9.1-22.04_edge
```  
...
### 本地启动  
```bash  
docker run -d --name kafka-container -e TZ=UTC -p 9092:9092 docker.xuanyuan.run/ubuntu/kafka:3.1-22.04_beta
```  
启动后，可通过 `[] 访问 Kafka 服务。  
