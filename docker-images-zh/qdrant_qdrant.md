...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/qdrant/qdrant" title="qdrant/qdrant Docker 镜像中文简介、标签列表与拉取命令">qdrant/qdrant 中文简介</a>
...
#### 基础运行命令
```bash
docker run -p 6333:6333 docker.xuanyuan.run/qdrant/qdrant
```
...
```bash
docker run -p 6333:6333 -v $(pwd)/qdrant_data:/qdrant/storage docker.xuanyuan.run/qdrant/qdrant
```
...
services:
  qdrant:
    image: docker.xuanyuan.run/qdrant/qdrant
    ports:
      - "6333:6333"
      - "6334:6334"  # gRPC端口
    volumes:
...
```python
from docker.xuanyuan.run/qdrant_client import QdrantClient
