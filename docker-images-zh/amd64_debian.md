...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/amd64/debian" title="amd64/debian Docker 镜像中文简介、标签列表与拉取命令">amd64/debian 中文简介</a>
...
#### 1. 运行交互式终端
```bash
docker run --rm -it docker.xuanyuan.run/amd64/debian:latest bash
```
- `--rm`：容器退出后自动删除
- `-it`：交互式终端模式
...
#### 2. 运行自定义命令
```bash
docker run --rm docker.xuanyuan.run/amd64/debian:bookworm-slim cat /etc/os-release
```
输出 Debian 版本信息。

#### 3. 挂载本地目录
```bash
docker run --rm -it -v $(pwd):/app docker.xuanyuan.run/amd64/debian:trixie-slim bash
```
...
services:
  debian-app:
    image: docker.xuanyuan.run/amd64/debian:bookworm
