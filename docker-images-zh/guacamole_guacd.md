...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/guacamole/guacd" title="guacamole/guacd Docker 镜像中文简介、标签列表与拉取命令">guacamole/guacd 中文简介</a>
...
```bash
docker run --name some-guacd -d docker.xuanyuan.run/guacamole/guacd
```
...
```bash
docker run --name some-guacd -d -p 4822:4822 docker.xuanyuan.run/guacamole/guacd
```
...
```bash
docker run --name some-app --link some-guacd:guacd -d docker.xuanyuan.run/application-that-uses-guacd
```
