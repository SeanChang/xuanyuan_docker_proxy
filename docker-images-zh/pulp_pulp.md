...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/pulp/pulp" title="pulp/pulp Docker 镜像中文简介、标签列表与拉取命令">pulp/pulp 中文简介</a>
...
#### 测试环境启动
```bash
docker run -d -p 8080:80 --name pulp3-full docker.xuanyuan.run/pulp/pulp3-with-all-plugins
```
...
docker run -d \
  ...
  --name pulp3-full \
  docker.xuanyuan.run/pulp/pulp3-with-all-plugins
```
...
services:
  pulp:
    image: docker.xuanyuan.run/pulp/pulp3-with-all-plugins
    ports:
      - "8080:80"
    volumes:
