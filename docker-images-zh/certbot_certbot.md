---
image: certbot/certbot
description: "这是电子前哨基金会（EFF）Certbot工具的官方版本，用于从非营利性证书颁发机构Let's Encrypt获取TLS/SSL证书，该工具旨在自动化证书的获取、安装及更新流程，帮助网站管理员轻松实现HTTPS加密，保障网络通信安全，是目前广泛使用的免费SSL证书管理工具之一。"
source: https://xuanyuan.cloud/zh/r/certbot/certbot
canonical: https://xuanyuan.cloud/zh/r/certbot/certbot
exported_at: 2026-06-02T12:04:49.546Z
---

**轩辕镜像中文简介（在线版）：** [certbot/certbot — 轩辕镜像中文简介](https://xuanyuan.cloud/zh/r/certbot/certbot)

含镜像标签、拉取命令、部署文档与相关推荐。

[certbot/certbot Docker 镜像中文简介、标签列表与拉取命令](https://xuanyuan.cloud/zh/r/certbot/certbot)

## Certbot 官方 Docker 仓库  


### 基本信息  
![当前版本：v1.7.0]([])  
![构建状态]([])  


### 仓库说明  
这是 Certbot 核心镜像的官方 Docker 仓库。  

关于该镜像的具体使用方法，可参考 [Certbot 官方文档 - Docker 安装指南]([])。  


### Certbot DNS 插件  
以下是 Certbot 支持的 DNS 插件及其 Docker 镜像地址：  
- [dns-dnsmadeeasy]([])  
- [dns-dnsimple]([])  
- [dns-ovh]([])  
- [dns-cloudflare]([])  
- [dns-cloudxns]([])  
- [dns-digitalocean]([])  
- [dns-google]([])  
- [dns-luadns]([])  
- [dns-nsone]([])  
- [dns-rfc2136]([])  
- [dns-route53]([])  
- [dns-gehirn]([])  
- [dns-linode]([])  
- [dns-sakuracloud]([])
