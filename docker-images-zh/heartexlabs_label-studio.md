...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/heartexlabs/label-studio" title="heartexlabs/label-studio Docker 镜像中文简介、标签列表与拉取命令">heartexlabs/label-studio 中文简介</a>
...
```bash
# 拉取最新镜像
docker pull docker.xuanyuan.run/heartexlabs/label-studio:latest

# 运行容器，映射8080端口并挂载数据卷
docker run -it -p 8080:8080 -v $(pwd)/mydata:/label-studio/data docker.xuanyuan.run/heartexlabs/label-studio:latest
```
...
```bash
docker run -it -p 8080:8080 -v $(pwd)/mydata:/label-studio/data docker.xuanyuan.run/heartexlabs/label-studio:latest label-studio --log-level DEBUG
```
