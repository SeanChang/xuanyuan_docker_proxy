...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/chainguard/node" title="chainguard/node Docker 镜像中文简介、标签列表与拉取命令">chainguard/node 中文简介</a>
...
```bash
docker pull docker.xuanyuan.run/chainguard/node:latest
```
...
```bash
docker run --rm -v $(pwd):/app docker.xuanyuan.run/chainguard/node:latest node /app/app.js
```
