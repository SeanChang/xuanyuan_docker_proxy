...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/jeessy/ddns-go" title="jeessy/ddns-go Docker 镜像中文简介、标签列表与拉取命令">jeessy/ddns-go 中文简介</a>
...
```bash
docker run -d --name ddns-go --restart=always --net=host -v /opt/ddns-go:/root docker.xuanyuan.run/jeessy/ddns-go
```  
启动后访问 `[] 配置。  
...
- **使用 ghcr.io 镜像**：  
  ```bash
  docker run -d --name ddns-go --restart=always --net=host -v /opt/ddns-go:/root ***-ghcr.xuanyuan.run/jeessy2/ddns-go
  ```  

- **自定义端口和间隔**：  
  ```bash
  docker run -d --name ddns-go --restart=always --net=host -v /opt/ddns-go:/root docker.xuanyuan.run/jeessy/ddns-go -l :9877 -f 600
  ```  

- **非 host 网络模式**：  
  ```bash
  docker run -d --name ddns-go --restart=always -p 9876:9876 -v /opt/ddns-go:/root docker.xuanyuan.run/jeessy/ddns-go
  ```  
