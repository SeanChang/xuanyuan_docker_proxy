...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/library/jenkins" title="library/jenkins Docker 镜像中文简介、标签列表与拉取命令">library/jenkins 中文简介</a>
...
```console
docker run -p 8080:8080 -p 50000:50000 docker.xuanyuan.run/jenkins
```  

Jenkins 数据（含插件、配置等）存储在 `/var/jenkins_home` 目录，建议通过卷挂载实现持久化：  

```console
docker run -p 8080:8080 -p 50000:50000 -v /your/home:/var/jenkins_home docker.xuanyuan.run/jenkins
```  
...
```console
docker run --name myjenkins -p 8080:8080 -p 50000:50000 -v /var/jenkins_home docker.xuanyuan.run/jenkins
```  
...
```dockerfile
FROM docker.xuanyuan.run/jenkins
...
```console
docker run --name myjenkins -p 8080:8080 -p 50000:50000 --env JAVA_OPTS=-Dhudson.footerURL=[] docker.xuanyuan.run/jenkins
```  
...
EOF
docker run --name myjenkins -p 8080:8080 -p 50000:50000 --env JAVA_OPTS="-Djava.util.logging.config.file=/var/jenkins_home/log.properties" -v `pwd`/data:/var/jenkins_home docker.xuanyuan.run/jenkins
```  
...
```console
docker run docker.xuanyuan.run/jenkins --version
```  

也可通过 `JENKINS_OPTS` 环境变量定义，例如强制使用 HTTPS：  

```dockerfile
FROM docker.xuanyuan.run/jenkins:1.565.3
...
```console
docker run --name myjenkins -p 8080:8080 -p 50001:50001 --env JENKINS_SLAVE_AGENT_PORT=50001 docker.xuanyuan.run/jenkins
```  
