...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/grafana/promtail" title="grafana/promtail Docker 镜像中文简介、标签列表与拉取命令">grafana/promtail 中文简介</a>
...
```bash
docker run --name some-promtail -v /path/to/config.yml:/etc/promtail/config.yml -v /var/log:/var/log -d docker.xuanyuan.run/grafana/promtail:tag --config.file=/etc/promtail/config.yml
```
...
services:
  loki:
    image: docker.xuanyuan.run/grafana/loki:latest
    ports:
      - 3100:3100
    volumes:
      - loki-data:/loki/data
  ...
  promtail:
    image: docker.xuanyuan.run/grafana/promtail:latest
    volumes:
