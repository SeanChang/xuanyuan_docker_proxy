...
```console
docker run --name redis-sentinel -e REDIS_MASTER_HOST=redis docker.xuanyuan.run/bitnami/redis-sentinel:latest
```
...
从 Docker Hub 拉取最新版：
```console
docker pull docker.xuanyuan.run/bitnami/redis-sentinel:latest
```

拉取特定版本（需替换 `[TAG]` 为具体版本号）：
```console
docker pull docker.xuanyuan.run/bitnami/redis-sentinel:[TAG]
```
...
services:
  ...
  redis-sentinel:
    image: docker.xuanyuan.run/bitnami/redis-sentinel:latest
    depends_on:
      - redis-master
    environment:
