...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/victoriametrics/victoria-metrics" title="victoriametrics/victoria-metrics Docker 镜像中文简介、标签列表与拉取命令">victoriametrics/victoria-metrics 中文简介</a>
...
```bash
docker run -d --name victoriametrics -p 8428:8428 docker.xuanyuan.run/victoriametrics/victoria-metrics:latest
```
...
docker run -d --name victoriametrics -p 8428:8428 \
  ...
  docker.xuanyuan.run/victoriametrics/victoria-metrics:latest \
...
```bash
docker run --rm docker.xuanyuan.run/victoriametrics/victoria-metrics:latest --help
```
