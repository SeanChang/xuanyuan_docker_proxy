...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/library/rabbitmq" title="library/rabbitmq Docker 镜像中文简介、标签列表与拉取命令">library/rabbitmq 中文简介</a>
...
```bash
docker run -d --hostname my-rabbit --name some-rabbit docker.xuanyuan.run/rabbitmq:3
```  
...
docker run -d --hostname my-rabbit --name some-rabbit \
  ...
  docker.xuanyuan.run/rabbitmq:3-management
```  
...
```bash
# 直接暴露管理端口到主机8080
docker run -d --hostname my-rabbit --name some-rabbit -p 8080:15672 docker.xuanyuan.run/rabbitmq:3-management
```  
...
```Dockerfile
FROM docker.xuanyuan.run/rabbitmq:3.8-management
