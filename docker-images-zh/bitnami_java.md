...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/bitnami/java" title="bitnami/java Docker 镜像中文简介、标签列表与拉取命令">bitnami/java 中文简介</a>
...
```console
docker run -it --name java docker.xuanyuan.run/bitnami/java
```
...
```console
docker pull docker.xuanyuan.run/bitnami/java:latest
```

如需使用特定版本，可拉取带版本标签的镜像。可在 [Docker Hub 仓库]  查看所有可用版本。

```console
docker pull docker.xuanyuan.run/bitnami/java:[TAG]
```
...
```console
docker run -it --name java -v /path/to/app:/app docker.xuanyuan.run/bitnami/java:latest \
...
```yaml
java:
  image: docker.xuanyuan.run/bitnami/java:latest
...
```console
docker run --name java docker.xuanyuan.run/bitnami/java:latest
```
