...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/apache/kafka" title="apache/kafka Docker 镜像中文简介、标签列表与拉取命令">apache/kafka 中文简介</a>
...
### 启动 Kafka broker
```console
docker run -d --name broker docker.xuanyuan.run/apache/kafka:latest
```
...
docker run -d  \
  ...
  docker.xuanyuan.run/apache/kafka:latest
```
...
services:
  broker:
    image: docker.xuanyuan.run/apache/kafka:latest
...
- **使用 `docker run`**：添加 `-p 9092:9092` 映射端口：
  ```console
  docker run -d -p 9092:9092 --name broker docker.xuanyuan.run/apache/kafka:latest
  ```
