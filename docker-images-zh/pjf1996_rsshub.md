...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/pjf1996/rsshub" title="pjf1996/rsshub Docker 镜像中文简介、标签列表与拉取命令">pjf1996/rsshub 中文简介</a>
...
```bash
# 拉取镜像
docker pull docker.xuanyuan.run/diygod/rsshub

# 运行容器
docker run -d --name rsshub -p 1200:1200 docker.xuanyuan.run/diygod/rsshub
```
...
docker run -d --name rsshub -p 1200:1200 \
  ...
  --link redis:redis \
  docker.xuanyuan.run/diygod/rsshub
```
