...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/linuxserver/rustdesk" title="linuxserver/rustdesk Docker 镜像中文简介、标签列表与拉取命令">linuxserver/rustdesk 中文简介</a>
...
services:  
  rustdesk:  
    image: docker.xuanyuan.run/linuxserver/rustdesk:latest  
    deploy:  
      resources:  
        reservations:  
          devices:  
...
docker run -d \  
  ...
  --shm-size="1gb" \  
  --restart unless-stopped \  
  docker.xuanyuan.run/linuxserver/rustdesk:latest
```  
...
```bash  
# 拉取新镜像  
docker pull docker.xuanyuan.run/linuxserver/rustdesk:latest  
# 停止并删除旧容器  
