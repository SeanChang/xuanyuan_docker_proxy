...
```bash
docker run --name <container-name> -d docker.xuanyuan.run/percona/percona-server-mongodb:<tag>
```  
...
```bash
docker run --name <container-name> -p 27017:27017 -d docker.xuanyuan.run/percona/percona-server-mongodb:<tag>
```  
...
```bash
docker run -it --link <container-name> --rm docker.xuanyuan.run/percona/percona-server-mongodb:<tag> mongosh mongodb://<mongodb-server>:<port>/<db-name>
```  

#### 版本 6.0 以下（使用 `mongo`）  
```bash
docker run -it --link <container-name> --rm docker.xuanyuan.run/percona/percona-server-mongodb:<tag> mongo mongodb://<mongodb-server>:<port>/<db-name>
```  
...
- 使用示例：  
  ...
  docker run -d --name <container-name> \
    ...
    docker.xuanyuan.run/percona/percona-server-mongodb:<tag>
  ```  
  连接数据库：  
  ```bash
...
```bash
docker run --name <container-name> --env-file <path/to/env-file> -d docker.xuanyuan.run/percona/percona-server-mongodb:<tag>
```  
...
2. 启动容器：  
   ```bash
   docker run --name <container-name> -v /local/datadir:/data/db -d docker.xuanyuan.run/percona/percona-server-mongodb:<tag>
   ```  
...
```bash
docker run --name <container-name> -d docker.xuanyuan.run/percona/percona-server-mongodb:<tag> --option1=value --option2=value
```  
示例（启用审计日志）：  
```bash
docker run --name <container-name> -d docker.xuanyuan.run/percona/percona-server-mongodb:6.0 --auditDestination=file --auditFormat=JSON --auditPath=/data/db/audit.log
```  
...
services:
  psmdb:
    image: docker.xuanyuan.run/percona/percona-server-mongodb:6.0
