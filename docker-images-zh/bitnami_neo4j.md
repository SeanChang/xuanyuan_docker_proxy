...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/bitnami/neo4j" title="bitnami/neo4j Docker 镜像中文简介、标签列表与拉取命令">bitnami/neo4j 中文简介</a>
...
```console
docker run --name neo4j docker.xuanyuan.run/bitnami/neo4j:latest
```
...
```console
docker pull docker.xuanyuan.run/bitnami/neo4j:latest
```

如需使用特定版本，可拉取带版本标签的镜像。可在 [Docker Hub Registry](https://hub.docker.com/r/bitnami/neo4j/tags/) 查看可用版本列表。

```console
docker pull docker.xuanyuan.run/bitnami/neo4j:[TAG]
```
...
docker run -d \
  ...
  docker.xuanyuan.run/bitnami/neo4j:latest
```
...
services:
  neo4j:
    image: docker.xuanyuan.run/bitnami/neo4j:latest
    ports:
      - 7474:7474
      - 7473:7473
      - 7687:7687
...
```console
docker run -v /path/to/neo4j-persistence:/bitnami docker.xuanyuan.run/bitnami/neo4j:latest
```
...
```console
docker run --name neo4j-node1 --network neo4j-network docker.xuanyuan.run/bitnami/neo4j:latest
```
...
```console
docker run -d -e NEO4J_BOLT_PORT_NUMBER=7777 --name neo4j docker.xuanyuan.run/bitnami/neo4j:latest
```
