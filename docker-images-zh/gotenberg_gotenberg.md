...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/gotenberg/gotenberg" title="gotenberg/gotenberg Docker 镜像中文简介、标签列表与拉取命令">gotenberg/gotenberg 中文简介</a>
...
#### 官方仓库镜像
```bash
docker run --rm -p 3000:3000 docker.xuanyuan.run/gotenberg/gotenberg:8
```

#### 赞助商仓库镜像（历史兼容）
由赞助商 TheCodingMachine 提供的镜像：
```bash
docker run --rm -p 3000:3000 docker.xuanyuan.run/thecodingmachine/gotenberg:8
```
...
services:
  gotenberg:
    image: docker.xuanyuan.run/gotenberg/gotenberg:8
...
docker run --rm -p 3000:3000 \
  -e LOG_LEVEL=debug \
  -e API_TIMEOUT=60 \
  docker.xuanyuan.run/gotenberg/gotenberg:8
```
