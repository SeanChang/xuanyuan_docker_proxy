...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/geonode/geoserver" title="geonode/geoserver Docker 镜像中文简介、标签列表与拉取命令">geonode/geoserver 中文简介</a>
...
```bash
docker pull docker.xuanyuan.run/geonode/geoserver
```
...
docker run --name "geoserver" \
  ...
  -d -p 8080:8080 \
  docker.xuanyuan.run/geonode/geoserver
```
...
#### 启动 PostGIS 容器
```bash
docker run -d --name="postgis" docker.xuanyuan.run/kartoza/postgis # 使用 kartoza/postgis 镜像
```
...
services:
  geoserver:
    image: docker.xuanyuan.run/geonode/geoserver:2.18.2
    ports:
      - "8080:8080"
    volumes:
  ...
  postgis:
    image: docker.xuanyuan.run/kartoza/postgis
    environment:
