...
> **轩辕镜像中文简介（在线版）**：[magiccoders/nginxpulse](https://xuanyuan.cloud/zh/r/magiccoders/nginxpulse)
...
> 本项目会访问外网 IP 归属地 API（默认 `ip-api.com`），部署环境需放行该域名的出站访问。同时也支持自己搭建IP归属地查询服务。
