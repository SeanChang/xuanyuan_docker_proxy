...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/1panel/maxkb" title="1panel/maxkb Docker 镜像中文简介、标签列表与拉取命令">1panel/maxkb 中文简介</a>
...
通过Docker快速部署：  
```bash
docker run -d --name=maxkb --restart=always -p 8080:8080 -v ~/.maxkb:/opt/maxkb docker.xuanyuan.run/1panel/maxkb
```  
- 默认用户名：admin  
- 默认密码：MaxKB@123..  
