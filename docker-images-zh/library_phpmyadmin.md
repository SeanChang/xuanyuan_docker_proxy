...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/library/phpmyadmin" title="library/phpmyadmin Docker 镜像中文简介、标签列表与拉取命令">library/phpmyadmin 中文简介</a>
...
```sh
docker run --name docker.xuanyuan.run/phpmyadmin -d --link mysql_db_server:db -p 8080:80 phpmyadmin
```
...
```sh
docker run --name docker.xuanyuan.run/phpmyadmin -d -e PMA_HOST=dbhost -p 8080:80 phpmyadmin
```
...
```sh
docker run --name docker.xuanyuan.run/phpmyadmin -d -e PMA_ARBITRARY=1 -p 8080:80 phpmyadmin
```
...
docker run --name phpmyadmin -d --link mysql_db_server:db -p 8080:80 \
  ...
  docker.xuanyuan.run/phpmyadmin
```
示例配置（启用 phpinfo 链接）：
```php
<?php
...
```sh
# 单服务器
docker run --name docker.xuanyuan.run/phpmyadmin -d -e PMA_HOST=sslhost -e PMA_SSL=1 -p 8080:80 phpmyadmin

# 多服务器（第一个启用 SSL，第二个不启用）
docker run --name docker.xuanyuan.run/phpmyadmin -d -e PMA_HOSTS='sslhost,nosslhost' -e PMA_SSLS='1,0' -p 8080:80 phpmyadmin
```
