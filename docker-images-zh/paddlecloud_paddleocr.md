...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/paddlecloud/paddleocr" title="paddlecloud/paddleocr Docker 镜像中文简介、标签列表与拉取命令">paddlecloud/paddleocr 中文简介</a>
...
```bash
docker run --name ppocr -v $PWD:/mnt -p 8888:8888 --shm-size=32g docker.xuanyuan.run/paddlecloud/paddleocr:2.5-cpu-latest
```  
...
#### 以bash模式创建容器  
```bash
docker run --name ppocr -v $PWD:/mnt -p 8888:8888 -it --shm-size=32g docker.xuanyuan.run/paddlecloud/paddleocr:2.5-cpu-latest /bin/bash
```  
...
#### 以jupyter模式创建容器  
```bash
docker run --name ppocr --runtime=nvidia -v $PWD:/mnt -p 8888:8888 --shm-size=32g docker.xuanyuan.run/paddlecloud/paddleocr:2.5-gpu-cuda10.2-cudnn7-latest
```  
...
#### 以bash模式创建容器  
```bash
docker run --name ppocr --runtime=nvidia -v $PWD:/mnt -p 8888:8888 -it --shm-size=32g docker.xuanyuan.run/paddlecloud/paddleocr:2.5-gpu-cuda10.2-cudnn7-latest /bin/bash
```  
