...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/bitnami/redis-exporter" title="bitnami/redis-exporter Docker 镜像中文简介、标签列表与拉取命令">bitnami/redis-exporter 中文简介</a>
...
```console
docker pull docker.xuanyuan.run/bitnami/redis-exporter:latest
```

如需使用特定版本，可拉取带版本标签的镜像。查看 [Docker Hub 标签列表](https://hub.docker.com/r/bitnami/redis-exporter/tags/) 获取可用版本：

```console
docker pull docker.xuanyuan.run/bitnami/redis-exporter:[TAG]
```
...
```console
docker run --name redis-exporter docker.xuanyuan.run/bitnami/redis-exporter:latest
```
...
```console
docker run --name redis-exporter-node1 --network redis-exporter-network docker.xuanyuan.run/bitnami/redis-exporter:latest
```
