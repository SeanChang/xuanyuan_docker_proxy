...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/amd64/ubuntu" title="amd64/ubuntu Docker 镜像中文简介、标签列表与拉取命令">amd64/ubuntu 中文简介</a>
...
```bash
# 拉取最新 LTS 版本
docker pull docker.xuanyuan.run/amd64/ubuntu:latest

# 拉取指定版本（如 22.04 LTS）
docker pull docker.xuanyuan.run/amd64/ubuntu:22.04

# 拉取滚动更新版本
docker pull docker.xuanyuan.run/amd64/ubuntu:rolling
```

#### 基本运行
```bash
# 交互式运行（进入 bash 终端）
docker run -it --rm docker.xuanyuan.run/amd64/ubuntu:latest bash
```
...
```bash
docker run -it --rm -e LANG=C.UTF-8 docker.xuanyuan.run/amd64/ubuntu:latest bash
```
...
services:
  ubuntu-app:
    image: docker.xuanyuan.run/amd64/ubuntu:latest
    tty: true  # 保持终端连接
    environment:
