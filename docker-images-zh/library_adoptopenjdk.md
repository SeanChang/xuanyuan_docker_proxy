...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/library/adoptopenjdk" title="library/adoptopenjdk Docker 镜像中文简介、标签列表与拉取命令">library/adoptopenjdk 中文简介</a>
...
创建 `Dockerfile`：
```dockerfile
FROM docker.xuanyuan.run/adoptopenjdk:11-jre-hotspot
RUN mkdir /opt/app
...
创建 `Dockerfile`：
```dockerfile
FROM docker.xuanyuan.run/adoptopenjdk:11-jre-openj9
RUN mkdir /opt/app
COPY japp.jar /opt/app
CMD ["java", "-jar", "/opt/app/japp.jar"]
```
...
创建 `Dockerfile`：
```dockerfile
FROM docker.xuanyuan.run/adoptopenjdk:12.0.1_12-jdk-openj9-0.14.1
CMD ["java", "-jar", "/opt/app/japp.jar"]
```
