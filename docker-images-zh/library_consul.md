...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/library/consul" title="library/consul Docker 镜像中文简介、标签列表与拉取命令">library/consul 中文简介</a>
...
#### 启动命令  
```console
docker run -d --name=dev-consul -e CONSUL_BIND_INTERFACE=eth0 docker.xuanyuan.run/consul
```  
...
```console
docker run -d -e CONSUL_BIND_INTERFACE=eth0 docker.xuanyuan.run/consul agent -dev -join=172.17.0.2 # 第二个节点
docker run -d -e CONSUL_BIND_INTERFACE=eth0 docker.xuanyuan.run/consul agent -dev -join=172.17.0.2 # 第三个节点
```  
...
docker run -d --net=host -e 'CONSUL_LOCAL_CONFIG={"leave_on_terminate": true}' \
  docker.xuanyuan.run/consul agent -bind=<外部IP> -retry-join=<服务器IP>
```  
- 参数说明：  
  - `--net=host`：使用主机网络。  
...
docker run -d --net=host -e 'CONSUL_LOCAL_CONFIG={"skip_leave_on_interrupt": true}' \
  docker.xuanyuan.run/consul agent -server -bind=<外部IP> -retry-join=<其他服务器IP> -bootstrap-expect=<服务器总数>
```  
- 参数说明：  
  - `-server`：启用服务器模式。  
...
docker run -d --net=host -e 'CONSUL_ALLOW_PRIVILEGED_PORTS=' \
  docker.xuanyuan.run/consul agent -dns-port=53 -recursor=8.8.8.8 -bind=<桥接IP>
```  
...
#### 其他容器使用 Consul DNS  
```console
docker run -i --dns=<桥接IP> -t docker.xuanyuan.run/ubuntu sh -c "apt-get update && apt-get install -y dnsutils && dig consul.service.consul"
```  
