...
1. 运行 Docker 命令：  
   ```bash
   docker run -ti -p 8080:8080 -p 29418:29418 docker.xuanyuan.run/gerritcodereview/gerrit
   ```  
...
```bash
docker run -ti -p 8080:8080 -p 29418:29418 docker.xuanyuan.run/gerritcodereview/gerrit:3.3.0
```  
...
services:
  gerrit:
    image: docker.xuanyuan.run/gerritcodereview/gerrit
    volumes:
...
services:
  ...
  ldap:
    image: docker.xuanyuan.run/osixia/openldap
    ports:
      - "389:389"
      - "636:636"
    environment:
  ...
  ldap-admin:
    image: docker.xuanyuan.run/osixia/phpldapadmin
    ports:
      - "6443:443"
    environment:
