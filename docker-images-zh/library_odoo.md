...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/library/odoo" title="library/odoo Docker 镜像中文简介、标签列表与拉取命令">library/odoo 中文简介</a>
...
services:
  web:
    image: docker.xuanyuan.run/odoo:17.0  # 指定 Odoo 版本
...
services:
  web:
    image: docker.xuanyuan.run/odoo:17.0
    depends_on: [mydb]
    ports: ["8069:8069"]
    environment:
