...
```console
docker run -it docker.xuanyuan.run/spark /opt/spark/bin/spark-shell
```  
示例代码（应返回 1000000000）：  
```scala
...
```console
docker run -it docker.xuanyuan.run/spark:python3 /opt/spark/bin/pyspark
```  
示例代码（应返回 1000000000）：  
```python
...
启动 SparkR 交互式 shell：  
```console
docker run -it docker.xuanyuan.run/spark:r /opt/spark/bin/sparkR
```  
