...
```dockerfile
FROM docker.xuanyuan.run/php:8.2-cli
COPY . /usr/src/myapp
...
以安装 GD 扩展为例（需先安装依赖库）：  
```dockerfile
FROM docker.xuanyuan.run/php:8.2-fpm
...
```dockerfile
FROM docker.xuanyuan.run/php:8.2-fpm-alpine
# 启用生产环境配置
...
```dockerfile
FROM docker.xuanyuan.run/php:7.2-apache
