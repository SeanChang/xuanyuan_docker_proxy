...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/myjobglasses/hermes" title="myjobglasses/hermes Docker 镜像中文简介、标签列表与拉取命令">myjobglasses/hermes 中文简介</a>
...
```dockerfile
# 继承Hermes API基础镜像
FROM docker.xuanyuan.run/hermes-api-base-image:latest
...
```bash
docker run --rm docker.xuanyuan.run/hermes-api-base-image:latest /bin/sh -c "echo 'Hermes API base environment is ready'"
```
