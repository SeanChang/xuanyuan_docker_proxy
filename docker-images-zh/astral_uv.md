...
FROM docker.xuanyuan.run/python:3.12-slim-bookworm
...
FROM docker.xuanyuan.run/astral/uv:python3.12-bookworm-slim
```
...
```bash
docker run -it --rm docker.xuanyuan.run/astral/uv:bookworm-slim uv --version
```
