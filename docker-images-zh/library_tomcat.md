...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/library/tomcat" title="library/tomcat Docker 镜像中文简介、标签列表与拉取命令">library/tomcat 中文简介</a>
...
```bash
docker run -it --rm docker.xuanyuan.run/tomcat:9.0
```  
...
```bash
docker run -it --rm -p 8888:8080 docker.xuanyuan.run/tomcat:9.0
```  
访问`[]  
...
docker run -it --rm -p 8888:8080 docker.xuanyuan.run/tomcat:9.0 sh -c "cp -r webapps.dist/* webapps && catalina.sh run"
```  
