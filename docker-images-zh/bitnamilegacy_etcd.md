...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/bitnamilegacy/etcd" title="bitnamilegacy/etcd Docker 镜像中文简介、标签列表与拉取命令">bitnamilegacy/etcd 中文简介</a>
...
```bash
docker pull docker.xuanyuan.run/bitnami/nginx:1.21.6-debian-10-r100
```  
...
```bash
docker run -d --name legacy-container docker.xuanyuan.run/bitnami/<image-name>:<tag>
```  
