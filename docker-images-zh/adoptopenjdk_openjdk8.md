...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/adoptopenjdk/openjdk8" title="adoptopenjdk/openjdk8 Docker 镜像中文简介、标签列表与拉取命令">adoptopenjdk/openjdk8 中文简介</a>
...
创建Dockerfile：  
```dockerfile
FROM docker.xuanyuan.run/adoptopenjdk/openjdk8:ubi  # 使用UBI系统的JDK镜像
RUN mkdir /opt/app
...
docker run -it --rm docker.xuanyuan.run/your-app # --rm：容器退出后自动删除
```  
...
```bash
docker run --rm -it docker.xuanyuan.run/adoptopenjdk/openjdk8:jre8u282-b08 java -version
```  
输出示例：  
```
