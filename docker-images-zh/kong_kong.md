...
### 基本运行
```bash
docker run -d --name kong-oss -p 8000:8000 -p 8443:8443 -p 8001:8001 -p 8444:8444 docker.xuanyuan.run/kong:latest
```
...
docker run -d --name kong-oss \
  ...
  docker.xuanyuan.run/kong:latest
```
...
docker run -d --name kong-oss \
  ...
  docker.xuanyuan.run/kong:latest kong start -c /etc/kong/kong.conf
```
