...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/cym1102/nginxwebui" title="cym1102/nginxwebui Docker 镜像中文简介、标签列表与拉取命令">cym1102/nginxwebui 中文简介</a>
...
### 2. 拉取镜像  
```bash  
docker pull docker.xuanyuan.run/cym1102/nginxwebui:latest  
```  

### 3. 启动容器  
```bash  
docker run -itd -v /home/nginxWebUI:/home/nginxWebUI -e BOOT_OPTIONS="--server.port=8080" --privileged=true --net=host docker.xuanyuan.run/cym1102/nginxwebui:latest
```  
...
services:  
  nginxWebUi-server:  
    image: docker.xuanyuan.run/cym1102/nginxwebui:latest  
    volumes:  
      - type: bind  
