...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/timberio/vector" title="timberio/vector Docker 镜像中文简介、标签列表与拉取命令">timberio/vector 中文简介</a>
...
只需使用新版本标签运行以下命令即可更新：  
```bash
docker pull docker.xuanyuan.run/timberio/vector:X.X.X-alpine && \
  docker run docker.xuanyuan.run/timberio/vector:X.X.X-alpine
```  
...
```bash
docker run docker.xuanyuan.run/timberio/vector:0.10.0-alpine
```  

#### debian  
基于[`debian-slim`镜像] 构建，是`debian`镜像的精简版。  
```bash
docker run docker.xuanyuan.run/timberio/vector:0.10.0-debian
```  
