...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/ubuntu/go" title="ubuntu/go Docker 镜像中文简介、标签列表与拉取命令">ubuntu/go 中文简介</a>
...
```bash
# 拉取最新稳定版
docker pull docker.xuanyuan.run/golang:latest

# 拉取指定版本（如1.21）的Alpine版（轻量级）
docker pull docker.xuanyuan.run/golang:1.21-alpine

# 拉取Debian基础版（包含更多系统工具）
docker pull docker.xuanyuan.run/golang:1.21-buster
```
...
```bash
docker run -it --rm docker.xuanyuan.run/golang:1.21-alpine sh
```
...
# 使用Go镜像编译程序
docker run --rm -v "$PWD":/app -w /app docker.xuanyuan.run/golang:1.21-alpine go build -o myapp main.go
```
...
```dockerfile
# 阶段1：编译阶段（使用完整Go环境）
FROM docker.xuanyuan.run/golang:1.21-alpine AS builder
WORKDIR /app
COPY go.mod go.sum ./
...
docker run -p 8080:8080 docker.xuanyuan.run/my-go-app
```
