...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/ubuntu/jre" title="ubuntu/jre Docker 镜像中文简介、标签列表与拉取命令">ubuntu/jre 中文简介</a>
...
#### 适用于 8 和 17 版本  
```bash
docker run -d --name jre-container -e TZ=UTC docker.xuanyuan.run/ubuntu/jre:17-22.04_edge
```  

#### 适用于 11 和 21 版本  
```bash
docker run -d --name jre-container -e TZ=UTC docker.xuanyuan.run/ubuntu/jre:21-24.04_edge exec java
```  
...
```dockerfile
# Dockerfile (8/17 版本)
FROM docker.xuanyuan.run/ubuntu:22.04 AS builder
RUN apt-get update && apt-get install -y openjdk-8-jdk
WORKDIR /app
ADD HelloWorld.java .
RUN javac -source 8 -target 8 HelloWorld.java -d .  # 编译 Java 文件

FROM docker.xuanyuan.run/ubuntu/jre:8-22.04_edge  # 使用 Chiselled Ubuntu JRE 镜像
WORKDIR /
...
FROM docker.xuanyuan.run/ubuntu/jre:21-24.04_edge
...
docker run --rm docker.xuanyuan.run/hello-jre # 输出 "Hello, World"
```  
