...
```console
docker run --name some-zabbix-java-gateway -d docker.xuanyuan.run/zabbix/zabbix-java-gateway:tag
```
...
```console
docker run --name some-zabbix-java-gateway --link some-zabbix-server:zabbix-server -d docker.xuanyuan.run/zabbix/zabbix-java-gateway:tag
```
...
services:
  ...
  zabbix-java-gateway:
    image: docker.xuanyuan.run/zabbix/zabbix-java-gateway:latest
    environment:
...
**示例**：调整轮询器数量和日志级别
```console
docker run --name some-zabbix-java-gateway -e ZBX_START_POLLERS=20 -e ZBX_DEBUGLEVEL=debug -d docker.xuanyuan.run/zabbix/zabbix-java-gateway:latest
```
...
**示例**：挂载本地JAR文件目录
```console
docker run --name some-zabbix-java-gateway -v /local/ext_lib:/usr/sbin/zabbix_java/ext_lib -d docker.xuanyuan.run/zabbix/zabbix-java-gateway:latest
```
