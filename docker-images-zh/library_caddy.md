...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/library/caddy" title="library/caddy Docker 镜像中文简介、标签列表与拉取命令">library/caddy 中文简介</a>
...
FROM docker.xuanyuan.run/caddy:<version>
...
```Dockerfile
FROM docker.xuanyuan.run/caddy:<version>-builder AS builder
...
services:
  caddy:
    image: docker.xuanyuan.run/caddy:<version>
