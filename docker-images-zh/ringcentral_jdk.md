...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/ringcentral/jdk" title="ringcentral/jdk Docker 镜像中文简介、标签列表与拉取命令">ringcentral/jdk 中文简介</a>
...
- **拉取命令**：  
  ```bash
  docker pull docker.xuanyuan.run/ringcentral/jdk:8u202
  ```  
...
- **拉取命令**：  
  ```bash
  docker pull docker.xuanyuan.run/ringcentral/maven:3.6.3-jdk11.0.7
  ```  
...
```Docker
FROM docker.xuanyuan.run/ringcentral/jdk:latest
COPY . /usr/src/myapp
...
拉取最新版镜像：  
```bash
docker pull docker.xuanyuan.run/ringcentral/jdk:latest
```  
