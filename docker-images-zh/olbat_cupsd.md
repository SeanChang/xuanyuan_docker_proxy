...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/olbat/cupsd" title="olbat/cupsd Docker 镜像中文简介、标签列表与拉取命令">olbat/cupsd 中文简介</a>
...
```bash
docker run -d -p 631:631 -v /var/run/dbus:/var/run/dbus --name cupsd docker.xuanyuan.run/olbat/cupsd
```

### 使用自定义配置  
如需自定义配置，将本地 `cupsd.conf` 文件挂载到容器内：  
```bash
docker run -d -p 631:631 -v /var/run/dbus:/var/run/dbus -v $PWD/cupsd.conf:/etc/cups/cupsd.conf --name cupsd docker.xuanyuan.run/olbat/cupsd
```
