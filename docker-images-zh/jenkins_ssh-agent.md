...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/jenkins/ssh-agent" title="jenkins/ssh-agent Docker 镜像中文简介、标签列表与拉取命令">jenkins/ssh-agent 中文简介</a>
...
```bash
docker run -d --rm --name=agent --publish 2200:22 -e "JENKINS_AGENT_SSH_PUBKEY=<公钥>" docker.xuanyuan.run/jenkins/ssh-agent
```  
...
```bash
docker run -v 自定义卷名:/home/jenkins/agent:rw docker.xuanyuan.run/jenkins/ssh-agent "<公钥>"
```  
...
```dockerfile
FROM docker.xuanyuan.run/jenkins/ssh-agent:debian-jdk17 as ssh-agent
# 添加自定义文件（如密钥），并确保权限正确
