...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/openeuler/sglang" title="openeuler/sglang Docker 镜像中文简介、标签列表与拉取命令">openeuler/sglang 中文简介</a>
...
### 拉取SGLang镜像
```bash
docker pull docker.xuanyuan.run/openeuler:sglang:{TAG}
```
...
```bash
docker run --rm --name sglang -p 8000:8000 --gpus all docker.xuanyuan.run/openeuler/sglang:{TAG} python3 -m sglang.launch_server --model-path {MODEL_PATH} --host 0.0.0.0 -port 8000
```
> 参数说明：
