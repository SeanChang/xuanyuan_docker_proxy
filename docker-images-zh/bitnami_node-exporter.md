...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/bitnami/node-exporter" title="bitnami/node-exporter Docker 镜像中文简介、标签列表与拉取命令">bitnami/node-exporter 中文简介</a>
...
```console
# 获取最新版
docker pull docker.xuanyuan.run/bitnami/node-exporter:latest

# 获取特定版本（需替换 [TAG] 为具体版本号）
docker pull docker.xuanyuan.run/bitnami/node-exporter:[TAG]
```

### 4.2 快速启动
基本部署命令：
```console
docker run --name node-exporter docker.xuanyuan.run/bitnami/node-exporter:latest
```
...
```console
docker run --name node-exporter --network node-exporter-network docker.xuanyuan.run/bitnami/node-exporter:latest
```
...
```console
docker run --name node-exporter docker.xuanyuan.run/bitnami/node-exporter:latest --collector.processes
```

#### 禁用默认收集器
通过 `--no-collector.<name>` 标志禁用默认启用的收集器：
```console
docker run --name node-exporter docker.xuanyuan.run/bitnami/node-exporter:latest --no-collector.loadavg
```
...
配置示例：
```console
docker run --name node-exporter -e OPENSSL_FIPS=no docker.xuanyuan.run/bitnami/node-exporter:latest
```
...
```console
docker run --name node-exporter --log-driver journald docker.xuanyuan.run/bitnami/node-exporter:latest
```
