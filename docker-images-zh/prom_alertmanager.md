...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/prom/alertmanager" title="prom/alertmanager Docker 镜像中文简介、标签列表与拉取命令">prom/alertmanager 中文简介</a>
...
```bash
docker run --name alertmanager -d -p 127.0.0.1:9093:9093 ***-quay.xuanyuan.run/prometheus/alertmanager
```
...
docker run --name alertmanager -d \
  ...
  ***-quay.xuanyuan.run/prometheus/alertmanager \
...
services:
  alertmanager:
    image: ***-quay.xuanyuan.run/prometheus/alertmanager
