...
```bash
docker pull docker.xuanyuan.run/igwn/ci-build-env:latest  # 最新稳定版
# 或指定版本标签（如基于Ubuntu 22.04的版本）
docker pull docker.xuanyuan.run/igwn/ci-build-env:ubuntu22.04-v1.5.0
```
...
services:
  igwn-ci-build:
    image: docker.xuanyuan.run/igwn/ci-build-env:ubuntu22.04-v1.5.0
    volumes:
...
docker run --rm \
  ...
  -v ./src:/workspace/src \
  docker.xuanyuan.run/igwn/ci-build-env:latest \
