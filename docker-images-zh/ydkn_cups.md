...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/ydkn/cups" title="ydkn/cups Docker 镜像中文简介、标签列表与拉取命令">ydkn/cups 中文简介</a>
...
```bash
docker run -d --restart always -p 631:631 -v $(pwd):/etc/cups docker.xuanyuan.run/ydkn/cups:latest
```
...
```bash
docker run -d --restart always -p 631:631 -v $(pwd):/etc/cups -e ADMIN_PASSWORD=mySecretPassword docker.xuanyuan.run/ydkn/cups:latest
```
