...
```bash  
docker run -d --name redis-stack -p 6379:6379 -p 8001:8001 -e REDIS_ARGS="--requirepass mypassword" docker.xuanyuan.run/redis/redis-stack:latest
```  
...
```bash  
docker run -v /local-data/:/data docker.xuanyuan.run/redis/redis-stack:latest
```  
...
```bash  
docker run -p 10001:6379 -p 13333:8001 docker.xuanyuan.run/redis/redis-stack:latest
```  
...
```bash  
docker run -v `pwd`/local-redis-stack.conf:/redis-stack.conf -p 6379:6379 -p 8001:8001 docker.xuanyuan.run/redis/redis-stack:latest
```  
...
- 通过 `REDIS_ARGS` 设置密码：  
  ```bash  
  docker run -e REDIS_ARGS="--requirepass redis-stack" docker.xuanyuan.run/redis/redis-stack:latest
  ```  
- 通过 `REDISTIMESERIES_ARGS` 设置数据保留策略（如保留 20 秒）：  
  ```bash  
  docker run -e REDISTIMESERIES_ARGS="RETENTION_POLICY=20" docker.xuanyuan.run/redis/redis-stack:latest
  ```
