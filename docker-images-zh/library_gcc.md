...
1. **编写Dockerfile**  
   ...
   ```dockerfile
   FROM docker.xuanyuan.run/gcc:4.9  # 可替换为其他支持的标签，如gcc:15
...
docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp docker.xuanyuan.run/gcc:4.9 gcc -o myapp myapp.c
```  
- `--rm`：编译完成后自动删除容器  
...
```bash
docker run --rm -v "$PWD":/usr/src/myapp -w /usr/src/myapp docker.xuanyuan.run/gcc:4.9 make
```  
