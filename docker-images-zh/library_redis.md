...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/library/redis" title="library/redis Docker 镜像中文简介、标签列表与拉取命令">library/redis 中文简介</a>
...
docker run -d \
  ...
  -v /opt/redis/data:/data \
  docker.xuanyuan.run/redis:latest redis-server /usr/local/etc/redis/redis.conf
```
...
docker run -it --rm --network some-network docker.xuanyuan.run/redis redis-cli -h redis
```
