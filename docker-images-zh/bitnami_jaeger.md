...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/bitnami/jaeger" title="bitnami/jaeger Docker 镜像中文简介、标签列表与拉取命令">bitnami/jaeger 中文简介</a>
...
```console
docker pull docker.xuanyuan.run/bitnami/jaeger:latest
```

如需使用特定版本，可拉取带版本标签的镜像（注意：2025 年 8 月 28 日后旧版本标签将迁移至 bitnamilegacy 仓库）：

```console
docker pull docker.xuanyuan.run/bitnami/jaeger:[TAG]
```
...
```console
docker run --name jaeger docker.xuanyuan.run/bitnami/jaeger:latest
```
...
services:
  jaeger:
    image: docker.xuanyuan.run/bitnami/jaeger:latest
    ports:
...
```console
docker run --rm --name jaeger docker.xuanyuan.run/bitnami/jaeger:latest --help
```
