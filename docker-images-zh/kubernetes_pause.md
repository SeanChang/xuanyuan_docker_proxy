...
### 4.1 镜像获取
```bash
docker pull docker.xuanyuan.run/[镜像仓库地址]/k8s-pause-restore:v1.0.0  # 替换为实际镜像仓库地址
```
...
docker run -it --rm \
  ...
  -e NAMESPACE="default" \
  -e LOG_LEVEL="debug" \
  docker.xuanyuan.run/[镜像仓库地址]/k8s-pause-restore:v1.0.0
```
...
services:
  pause-restore:
    image: docker.xuanyuan.run/[镜像仓库地址]/k8s-pause-restore:v1.0.0
    privileged: true
    volumes:
