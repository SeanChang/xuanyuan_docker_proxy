...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/library/ibmjava" title="library/ibmjava Docker 镜像中文简介、标签列表与拉取命令">library/ibmjava 中文简介</a>
...
FROM docker.xuanyuan.run/library/ibmjava:8-jre
...
# 交互式运行，退出时自动删除容器
docker run -it --rm -p 8080:8080 docker.xuanyuan.run/ibmjava-app
```
...
FROM docker.xuanyuan.run/library/ibmjava:8-sfj
...
# 挂载主机目录，实时读取最新 JAR 包
docker run -it --rm -v $(pwd)/jars:/opt/app -p 8080:8080 docker.xuanyuan.run/ibmjava-sfj-app
```
...
# 3. 启动多个容器，共享同一类缓存卷
docker run -d -v ibmjava-cds-volume:/opt/shareclasses -p 8081:8080 docker.xuanyuan.run/ibmjava-cds-app
docker run -d -v ibmjava-cds-volume:/opt/shareclasses -p 8082:8080 docker.xuanyuan.run/ibmjava-cds-app
```
