...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/library/emqx" title="library/emqx Docker 镜像中文简介、标签列表与拉取命令">library/emqx 中文简介</a>
...
```bash
docker run -d --name emqx -p 18083:18083 -p 1883:1883 docker.xuanyuan.run/emqx:latest
```  
...
```bash
docker run -d --name emqx -e EMQX_DASHBOARD__DEFAULT_PASSWORD=mysecret -p 18083:18083 -p 1883:1883 docker.xuanyuan.run/emqx:latest
```  
更多配置细节见 [官方文档]()。
...
services:
  emqx1:
    image: docker.xuanyuan.run/emqx:latest
    environment:
...
docker run -d --name emqx -p 18083:18083 -p 1883:1883 \
  ...
  docker.xuanyuan.run/emqx:latest
```  
