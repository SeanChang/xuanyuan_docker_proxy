...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/grafana/loki" title="grafana/loki Docker 镜像中文简介、标签列表与拉取命令">grafana/loki 中文简介</a>
...
```bash
docker run -d --name=loki -p 3100:3100 docker.xuanyuan.run/grafana/loki
```
...
```bash
docker run -d --name=loki --mount source=loki-data,target=/loki -p 3100:3100 docker.xuanyuan.run/grafana/loki
```
...
```bash
docker run -d --name=loki --mount type=bind,source="本地配置文件路径",target=/etc/loki/local-config.yaml -p 3100:3100 docker.xuanyuan.run/grafana/loki
```
...
```bash
docker run -d --name=loki -p 3100:3100 docker.xuanyuan.run/grafana/loki:1.4.1
```
