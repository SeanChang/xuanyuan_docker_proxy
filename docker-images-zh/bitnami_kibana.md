...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/bitnami/kibana" title="bitnami/kibana Docker 镜像中文简介、标签列表与拉取命令">bitnami/kibana 中文简介</a>
...
```console
docker run --name kibana docker.xuanyuan.run/bitnami/kibana:latest
```
...
2. 启动 Elasticsearch 容器：
   ...
   ```console
   docker run -d -p 9200:9200 --name elasticsearch --net=kibana_network docker.xuanyuan.run/bitnami/elasticsearch
   ```
...
3. 启动 Kibana 容器：
   ...
   docker run -d -p 5601:5601 --name kibana --net=kibana_network \
     ...
     docker.xuanyuan.run/bitnami/kibana
   ```
...
```console
docker run -v /本地路径/kibana-persistence:/bitnami/kibana docker.xuanyuan.run/bitnami/kibana:latest
```
...
2. 启动 Kibana 服务实例：
   ...
   docker run -d --name kibana-server \
     --network app-tier \
     docker.xuanyuan.run/bitnami/kibana:latest
   ```
...
```console
docker run -d -e KIBANA_ELASTICSEARCH_URL=elasticsearch --name kibana docker.xuanyuan.run/bitnami/kibana:latest
```
...
```console
docker run -v /本地脚本目录:/docker-entrypoint-initdb.d docker.xuanyuan.run/bitnami/kibana:latest
```
...
1. 启动容器并挂载持久化卷：
   ...
   ```console
   docker run --name kibana -v /本地路径/kibana-persistence:/bitnami docker.xuanyuan.run/bitnami/kibana:latest
   ```
...
1. 拉取最新镜像：
   ...
   ```console
   docker pull docker.xuanyuan.run/bitnami/kibana:latest
   ```
