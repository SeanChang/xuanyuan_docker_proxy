...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/xubiaolin/zerotier-planet" title="xubiaolin/zerotier-planet Docker 镜像中文简介、标签列表与拉取命令">xubiaolin/zerotier-planet 中文简介</a>
...
- **直接拉取**：从Docker Hub拉取（推荐，无需本地构建）：  
  ```bash
  docker pull docker.xuanyuan.run/:latest
  ```  
...
2. 新建 `docker-compose.yml` 文件，参考以下示例（具体参数可调整）：  
   ...
   services:
     zerotier-planet:
       image: docker.xuanyuan.run/:latest
...
- **更新镜像**：若需升级，先停止旧容器，拉取新版本镜像后重启：  
  ...
  docker pull docker.xuanyuan.run/:latest  # 拉取新版本
