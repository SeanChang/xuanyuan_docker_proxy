...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/docker/dockerfile" title="docker/dockerfile Docker 镜像中文简介、标签列表与拉取命令">docker/dockerfile 中文简介</a>
...
```dockerfile
# syntax=docker/dockerfile:1.4
FROM docker.xuanyuan.run/alpine
COPY --link /foo /bar
```  
等效于分别构建两个镜像后合并层：  
```dockerfile
...
```dockerfile
# syntax=docker/dockerfile:1.3
FROM docker.xuanyuan.run/golang
...
```dockerfile
# syntax=docker/dockerfile:1.3
FROM docker.xuanyuan.run/ubuntu
# 保留下载的 apt 包
...
```dockerfile
# syntax=docker/dockerfile:1.3
FROM docker.xuanyuan.run/python:3
...
```dockerfile
# syntax=docker/dockerfile:1.3
FROM docker.xuanyuan.run/python:3.6
...
```dockerfile
# syntax=docker/dockerfile:1.4
FROM docker.xuanyuan.run/debian
