...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/openspeedtest/latest" title="openspeedtest/latest Docker 镜像中文简介、标签列表与拉取命令">openspeedtest/latest 中文简介</a>
   ...
   services:  
     speedtest:  
       ...
       ports:  
         - '3000:3000'  # HTTP端口  
         - '3001:3001'  # HTTPS端口  
       image: docker.xuanyuan.run/openspeedtest/latest  
   ```  
...
- **自动配置SSL（Let's Encrypt）**：  
  ...
  ```bash  
  docker run -e ENABLE_LETSENCRYPT=True -e DOMAIN_NAME=speedtest.yourdomain.com -e USER_EMAIL=[邮箱已删除] --restart=unless-stopped --name openspeedtest -d -p 80:3000 -p 443:3001 docker.xuanyuan.run/openspeedtest/latest
  ```  
- **自定义SSL证书**：  
