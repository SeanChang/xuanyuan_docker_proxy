...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/mysql/mysql-router" title="mysql/mysql-router Docker 镜像中文简介、标签列表与拉取命令">mysql/mysql-router 中文简介</a>
...
```
docker run -e MYSQL_HOST=localhost -e MYSQL_PORT=3306 -e MYSQL_USER=mysql -e MYSQL_PASSWORD=mysql -e MYSQL_INNODB_CLUSTER_MEMBERS=3 -ti docker.xuanyuan.run/mysql/mysql-router
```
