...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/continuumio/miniconda3" title="continuumio/miniconda3 Docker 镜像中文简介、标签列表与拉取命令">continuumio/miniconda3 中文简介</a>
...
```bash
docker pull docker.xuanyuan.run/continuumio/miniconda3
docker run -i -t docker.xuanyuan.run/continuumio/miniconda3 /bin/bash
```
...
```bash
docker run -i -t -p 8888:8888 docker.xuanyuan.run/continuumio/miniconda3 /bin/bash -c "
