...
docker pull docker.xuanyuan.run/library/sapmachine:latest

# 运行容器并查看 Java 版本
docker run -it --rm docker.xuanyuan.run/library/sapmachine:latest java -version
...
FROM docker.xuanyuan.run/library/sapmachine:17-jdk-ubuntu-jammy
...
# 后台运行容器（映射端口 8080）
docker run -d -p 8080:8080 --name my-app-container docker.xuanyuan.run/my-java-app
```
...
FROM docker.xuanyuan.run/library/sapmachine:21-jre-headless-alpine-3.22
...
docker run -it --rm -p 8080:8080 docker.xuanyuan.run/my-lightweight-app
```
