...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/library/eclipse-temurin" title="library/eclipse-temurin Docker 镜像中文简介、标签列表与拉取命令">library/eclipse-temurin 中文简介</a>
...
**Dockerfile**：  
```dockerfile
FROM docker.xuanyuan.run/eclipse-temurin:21
RUN mkdir /opt/app
...
```console
docker build -t japp .
docker run -it --rm docker.xuanyuan.run/japp
```
...
```dockerfile
# 示例
FROM docker.xuanyuan.run/<base image>
...
```dockerfile
# 阶段 1：构建 JRE
FROM docker.xuanyuan.run/eclipse-temurin:21 as jre-build
...
# 阶段 2：部署应用
FROM docker.xuanyuan.run/debian:buster-slim
ENV JAVA_HOME=/opt/java/openjdk
ENV PATH "${JAVA_HOME}/bin:${PATH}"
...
**Dockerfile**：  
```dockerfile
FROM docker.xuanyuan.run/eclipse-temurin:21.0.2_13-jdk
CMD ["java", "-jar", "/opt/app/japp.jar"]
```
...
```console
docker build -t japp .
docker run -it -v /path/on/host/system/jars:/opt/app docker.xuanyuan.run/japp
```
