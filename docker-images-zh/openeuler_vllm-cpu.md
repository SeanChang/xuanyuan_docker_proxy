...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/openeuler/vllm-cpu" title="openeuler/vllm-cpu Docker 镜像中文简介、标签列表与拉取命令">openeuler/vllm-cpu 中文简介</a>
...
```bash
# 启动vLLM容器，映射8000端口
docker run --rm --name vllm -p 8000:8000 -it --entrypoint bash docker.xuanyuan.run/openeuler/vllm-cpu:latest
```
