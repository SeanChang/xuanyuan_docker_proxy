...
**轩辕镜像中文简介（在线版）：** <a href="https://xuanyuan.cloud/zh/r/bitnami/mariadb" title="bitnami/mariadb Docker 镜像中文简介、标签列表与拉取命令">bitnami/mariadb 中文简介</a>
...
```console
docker run --name mariadb -e ALLOW_EMPTY_PASSWORD=yes docker.xuanyuan.run/bitnami/mariadb:latest
```
...
```console
docker pull docker.xuanyuan.run/bitnami/mariadb:latest  # 拉取最新版
```  
如需指定版本，可使用标签拉取（[可用版本列表] ）：  
```console
docker pull docker.xuanyuan.run/bitnami/mariadb:[TAG]  # 将 [TAG] 替换为具体版本，如 11.4.2
```
...
docker run -it --rm \
    --network app-tier \
    docker.xuanyuan.run/bitnami/mariadb:latest mysql -h mariadb-server -u root
```
